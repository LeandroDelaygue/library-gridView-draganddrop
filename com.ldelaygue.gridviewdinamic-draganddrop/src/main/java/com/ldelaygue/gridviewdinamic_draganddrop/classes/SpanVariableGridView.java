...
import com.ldelaygue.gridviewdinamic.draganddrop.R;
...
public class SpanVariableGridView extends AdapterView<BaseAdapter> {
    ...
    private void initialize(final AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            try {
                mColCount = a.getInteger(R.styleable.SpanVariableGridView_numColumns, 1);
    ...
    protected Rect layoutChildrens(final int draggedChild, final boolean animate) {
        ...
        for (int position = 0; position < mAdapter.getCount(); position++) {
            ...
            final int width_ = column == LayoutParams.ALL_COLUMNS ? width : (width / 2) - mItemMargin;//(lp.span * (colWidth + mItemMargin) - mItemMargin);
